<template>
    <div id="">
      ...
      <!--内容-->
      <div class="search-city"><input type="text" placeholder="Enter name to filter" v-model="citySearch"></div>
      <div>
        ...
        this.$http.get('./static/city.json').then(response => {
         ...
         //   cityLists = cityList[i].firstName
          for (let j in cityLists) {
            let fullname = cityLists[j].firstName + ' ' + cityLists[j].lastName
            cityNames.push(fullname)
          }
        //  }
