...
<script>
  ...
  const buttonsIcon = [
    {title: 'traffic', click: false, isActive: true},
    {title: 'life', click: false, isActive: false},
    {title: 'Medical', click: false, isActive: false},
    {title: 'School', click: false, isActive: false}
  ]
  var mapListText = []
    ...
    methods: {
      init: function () {
        map = new AMap.Map('container', {
          center: [40.9128704, 29.298896600000035],
          resizeEnable: false,
          zoom: 5
        })
      ...
      searchData: function (callback) {
        let keyWords = ['Subway line', '大型购物广场', '三甲医院', '学校']
      ...
      demo: function () {
        ...
        if (numIndex > 4) {
          if (self.num.length > 0) {
            return map.clearMap()  // Clear the map cover
          }
        }
      },
      /*  Registration of each click event, the default display num0, that is, traffic, in fact, all the data has been requested, click the button just to switch maker */
      clickItem: function (index, buttons) {
        map.clearMap()  // Clear the map cover
